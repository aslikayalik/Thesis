...
def process_text(text):
    ...
    noun_cooperation = find_noun_cooperation(doc) #pencere_camı
    word_list = [token for token in noun_cooperation]
    results = list(prolog.query(f'sr_parse([],{word_list})'))
    return noun_cooperation, results

noun_cooperation, results = process_text("Ayşe kuş kafesi aldı.")
...
print(return_words_for_noun_cooperation(words_for_noun_cooperation))